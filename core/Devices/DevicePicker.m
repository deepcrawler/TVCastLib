...
- (instancetype) init
{
    ...
    if (self)
    {
        _sortQueue = dispatch_queue_create("TVCastLib Device Picker Sort", DISPATCH_QUEUE_SERIAL);
...
- (void) showPicker:(id)sender
{
    ...
    _showServiceLabel = false; //[DiscoveryManager sharedManager].capabilityFilters.count == 0;

    NSString *pickerTitle = [[NSBundle mainBundle] localizedStringForKey:@"TVCast_LIB_Search_Title" value:@"Pick a device" table:@"TVCastLib"];
...
{
    NSString *pickerTitle = [[NSBundle mainBundle] localizedStringForKey:@"TVCast_LIB_Search_Title" value:@"Pick a device" table:@"TVCastLib"];
    NSString *pickerCancel = [[NSBundle mainBundle] localizedStringForKey:@"TVCast_LIB_Search_Cancel" value:@"Cancel" table:@"TVCastLib"];
...
- (NSString *) nameForDevice:(ConnectableDevice *)device
{
    ...
    if (device.serviceDescription.friendlyName && device.serviceDescription.friendlyName.length > 0)
        ...
        name = [[NSBundle mainBundle] localizedStringForKey:@"TVCast_LIB_Unnamed_Device" value:@"Unnamed device" table:@"TVCastLib"];
