...
- (void) connect
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(disconnect) name:kTVCastLibWirelessSSIDChanged object:nil];
}
...
- (void) disconnect
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kTVCastLibWirelessSSIDChanged object:nil];
...
{
    if([service isKindOfClass:[DIALService class]] && [_services count]>0)
        return; //Bypass
    if(![service isKindOfClass:[DIALService class]])
    {
        ...
        {
            if([service isKindOfClass:[DIALService class]])
                [self removeServiceWithId:service.serviceName];
        }];
    }
    //
