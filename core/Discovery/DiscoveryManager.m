...
{
    NSDictionary *defaultPlatforms = kTVCastLibDefaultPlatforms;
...
- (void) detectSSIDChange
{
    ...
    if ([ssidName caseInsensitiveCompare:_currentSSID] != NSOrderedSame)
    {
        if (_currentSSID != nil)
        {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:kTVCastLibWirelessSSIDChanged object:nil];
        }
...
- (BOOL) descriptionIsNetcastTV:(ServiceDescription *)description
{
    ...
    if ([description.modelName.uppercaseString isEqualToString:@"LG TV"])
    {
        ...
        {
            isNetcast = [description.serviceId isEqualToString:kTVCastLibNetcastTVServiceId];
        }
    }
